...
function authenticateMFUsingUserStore(){
                ...
                var authParams = {"userid": mobileFabricConfigurationForIndigenousCancelRequest.identityServices[0].username, "password": mobileFabricConfigurationForIndigenousCancelRequest.identityServices[0].password};
...
function setIndigenousCancelRequest()
{
     ...
     // alert("operation name"+operationName);
                  ...
                  dataCancelRequest["requestId"]=businessCancelReqID;//1;//kony.store.getItem("requestId");
                  //dataCancelRequest["message"]="MerinnnnReal";
                  var rawReqdata=mainPage.cancelRequestModalContainer.requestModalBody.cancelRequestTextArea.text;
                  var modifiedReqdata=rawReqdata.replace(/(\r\n|\n|\r)/gm," ");
                 dataCancelRequest["message"]=modifiedReqdata;//mainPage.cancelRequestModalContainer.requestModalBody.cancelRequestTextArea.text;
...
function saveReqForCancel()
{
  if(kony.store.getItem("isBusOrVol")=="business")
  {
   
   //alert("the row is="+JSON.stringify(val));
    var busCancelVal=mainPage.mainPageBody.mainPageContentParent.myActivity.myAcceptedRequests.acceptedRequestsSegment.selectedRowItems;
    businessCancelReqID=busCancelVal[0].lblRequestId;
    //alert("id="+JSON.stringify(busCancelVal[0].lblRequestId));
   }

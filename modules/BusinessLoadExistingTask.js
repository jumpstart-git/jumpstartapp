...
function getBusinessLoadExistingTaskTaskSuccessCallback (gblBusinessLoadExistingTaskTaskList)
{
 ...
 // alert("inside getBusinessLoadExistingTaskTaskSuccessCallback"+JSON.stringify(gblBusinessLoadExistingTaskTaskList));
  ...
  if (gblBusinessLoadExistingTaskTaskList.taskList!=null && gblBusinessLoadExistingTaskTaskList.taskList!=undefined)
  {//second if
    if (gblBusinessLoadExistingTaskTaskList.taskList[0].TasksDTO !== null && gblBusinessLoadExistingTaskTaskList.taskList[0].TasksDTO!=undefined)
    {//3rd if
       // alert("2");
      // Making sure we have at least 1 article returned
      if (gblBusinessLoadExistingTaskTaskList.taskList.length > 0)
      {//4th if
...
//          kony.application.dismissLoadingScreen();
    ...
    kony.ui.Alert({ message: "Service call failed with opstatus " + BusinessLoadExistingTaskTaskList.opstatus,alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Fox News",yesLabel:"OK"}, {});
  }
}//end first if
}//end of fun

...
function initializeMobileFabricForBusOpenTaskExistingSuccess(response) { //alert("mf Success"); 
...
function getBusOpenTaskExistingSuccessCallback(gblBusOpenTaskExistingList) {
    ...
    if (gblBusOpenTaskExistingList !== null && gblBusOpenTaskExistingList.opstatus === 0) { //first if
        if (gblBusOpenTaskExistingList.taskList != null && gblBusOpenTaskExistingList.taskList != undefined) { //second if
            if (gblBusOpenTaskExistingList.taskList[0].TasksDTO !== null && gblBusOpenTaskExistingList.taskList[0].TasksDTO != undefined) { //3rd if
                // alert("2");
                // Making sure we have at least 1 article returned
                if (gblBusOpenTaskExistingList.taskList.length > 0) { //4th if

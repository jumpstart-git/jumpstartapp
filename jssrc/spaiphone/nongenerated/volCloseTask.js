...
function getVolCloseTaskSuccessCallback(gbVolCloseTaskList) {
    ...
    if (gbVolCloseTaskList !== null && gbVolCloseTaskList.opstatus === 0) { //first if
        if (gbVolCloseTaskList.taskList != null && gbVolCloseTaskList.taskList != undefined) { //second if
            if (gbVolCloseTaskList.taskList[0].TasksDTO !== null && gbVolCloseTaskList.taskList[0].TasksDTO != undefined) { //3rd if
                // alert("2");
                // Making sure we have at least 1 article returned
                if (gbVolCloseTaskList.taskList.length > 0) { //4th if

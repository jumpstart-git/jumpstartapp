function getPushNote() {
    try {
        //      alert("inside getPushNote111");
        // alert("hii");
        //var selectedKey = frmFoxNews.lstNewsType.selectedKey;
        if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
            // alert("Testing waters on the Push notification :) . The app might not be working right really.");
            var object = {
            ...
            // alert("Testing waters on the Push notification two :) . The app might not be working right really. Not sure..");
        } else {
...
function onsuccess(identifier) {
    try {
        //alert("Registered SUCCESSFULLY :"+identifier);
...
function pushSubscription(regId, ostype) {
    try {
        ...
        try {
            // kony.application.showLoadingScreen("sknLoading","Please wait...",constants.LOADING_SCREEN_POSITION_FULL_SCREEN, true, true,null);
            var connHandle = kony.net.invokeServiceAsync(pushSubscUrl, inputParamTable, asyncCallback1);
        } catch (err) {
            //  alert("\nexception in invoking service---\n"+JSON.stringify(err));
            alert("Error" + err);
            kony.application.dismissLoadingScreen();

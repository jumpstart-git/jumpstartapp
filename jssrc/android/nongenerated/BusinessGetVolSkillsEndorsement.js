...
function initializeMobileFabricGetVolunteerSkillsForEndorsementSuccess(response) { //alert("mf Success"); 
...
function getVolunteerSkillsForEndorsementSuccessCallback(gblVolSkillsResponse) {
    ...
    if (gblVolSkillsResponse !== null && gblVolSkillsResponse.opstatus === 0) { //first if
        if (gblVolSkillsResponse.volunteersList != null && gblVolSkillsResponse.volunteersList != undefined) { //second if
            if (gblVolSkillsResponse.volunteersList[0].VolunteersDTO !== null && gblVolSkillsResponse.volunteersList[0].VolunteersDTO != undefined) { //3rd if
                // alert("2");
                // Making sure we have at least 1 article returned
                if (gblVolSkillsResponse.volunteersList.length > 0) { //4th if

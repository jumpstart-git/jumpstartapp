...
function setIndigenousDeclineRequest() {
    ...
    if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
        ...
        dataDeclineRequest["message"] = mainPage.declineRequestModalContainer.declineRequestModalBody.declineRequestTextArea.text;
...
function saveReqForBusDecline(valDecline) {
    try {
        if (kony.store.getItem("isBusOrVol") == "business") {
            // alert("the row is="+JSON.stringify(valDecline));
            //gblReq=val.widgetInfo.data[0].lblRequestId;
            gblDeclineBusReq = valDecline.widgetInfo.selectedRowItems[0]["lblRequestIdSent"];
        }
    } catch (e) {
